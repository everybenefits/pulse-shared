...
export function canAssignRoleByAuthority(options: {
  ...
  const targetNormalized =
    options.targetRole === "teacher" ? "instructor" : options.targetRole.trim();
  ...
  if (isBuiltinRoleId(targetNormalized)) return true;
...
> = {
  ...
  agency_owner: {
    name: "Agency owner",
    description:
      "Owns one or more agencies; self-service agency management in Pulse Web.",
    category: "sales",
    sortOrder: 25,
    locked: false,
    editableBySystemOnly: true,
